"""World Bank project sites service — loads and serves WB project GeoJSON for Nigeria."""
...
BOUNDARIES_PATH = DATA_DIR / "nga_lga_boundaries.geojson"
CACHE_TTL = 3600
...
def _load_merged() -> pd.DataFrame:
    """Load, merge, and spatially enrich WB geo + master CSVs for Nigeria. Cached."""
    now = time.time()
    ...
    nga_geo = geo[geo["ISO_CNTRY_CODE"] == "NG"].copy()
    nga_geo["GEO_LATITUDE_NBR"] = pd.to_numeric(nga_geo["GEO_LATITUDE_NBR"], errors="coerce")
    nga_geo["GEO_LONGITUDE_NBR"] = pd.to_numeric(nga_geo["GEO_LONGITUDE_NBR"], errors="coerce")
    nga_geo = nga_geo.dropna(subset=["GEO_LATITUDE_NBR", "GEO_LONGITUDE_NBR"])

    nga_master = (
        master[master["CNTRY_SHORT_NAME"].str.contains("Nigeria", na=False)][_MASTER_COLS]
        .drop_duplicates("PROJ_ID")
    )

    merged = nga_geo.merge(nga_master, on="PROJ_ID", how="left")

    # Spatial join: assign state + LGA from boundary polygons based on coordinates.
    # This is necessary because active projects often have no ADMIN_UNIT1/2_NAME filled in.
    try:
        boundaries = gpd.read_file(BOUNDARIES_PATH)[["statename", "lganame", "geometry"]]
        ...
        merged["matched_admin1"] = joined["statename"].values
        merged["matched_admin2"] = joined["lganame"].values
...
def load_wb_projects(status_filter: str | None = None) -> dict:
    """Return a GeoJSON FeatureCollection of WB project sites in Nigeria."""
    now = time.time()
...
def get_projects_by_unit(level: int, name: str, status_filter: str | None = None) -> list[dict]:
    ...
    # Strip common suffixes so "Kano State" → "Kano" for matching
    name_clean = name.replace(" State", "").replace(" LGA", "").strip()
    ...
    if level == 2:
        ...
        # level 1 (state) or level 3 (ward — match to parent state)
        mask = merged["matched_admin1"].str.contains(name_clean, case=False, na=False)
...
def get_wb_state_summary() -> dict[str, dict]:
    """Return per-state aggregated WB project stats.

    Returns a dict keyed by state name with:
